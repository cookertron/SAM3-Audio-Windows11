...
1.  Run **`setup_windows.bat`**.
    ...
    *   Installs CUDA-enabled PyTorch and all required dependencies.
    *   Patches the local package for Windows compatibility.

3.  **Important**: If you received this as a portable package without the large model file:
    *   Download `checkpoint.pt` from the [Hugging Face Repo](https://huggingface.co/facebook/sam-audio-large/resolve/main/checkpoint.pt).
    *   Place it in: `checkpoints\sam-audio-large\checkpoint.pt`.

4.  Run the GUI application:
    *   Double-click **`start_gui.bat`**.

Or run the extraction script manually:
```bash
# Activate environment
venv\Scripts\activate
# Run extraction
python extract_vocals.py your_audio_file.wav
```
